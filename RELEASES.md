...
- **Default allowlist:** includes **`.devcontainer/**`** and **`.devcontainer.json`** so `./hframe in` (rsync with `--delete`) does not remove devcontainer metadata from the workspace; without them, “Reopen in Container” breaks after the first sync. Existing layouts: add those lines to `.hframe/policy.allowlist` (or widen policy) and run `./hframe in` again from the protected copy.
...
- **Policy:** default **allowlist** via `.hframe/policy.allowlist`; optional **`.hframe/policy.denylist`** merged after built-in denies; optional **`# hframe-policy: mode denylist-only`** with **`git add -A`** on export (see README).
